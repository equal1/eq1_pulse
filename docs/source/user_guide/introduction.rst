...
* Arithmetic, logical and comparison operations (e.g., `+`, `*`, `==`, `>`)
* JSON serialization/deserialization
* Schema generation for API generation
...
.. code-block:: python
    ...
    with build_sequence() as seq:
        ...
        # Play another pulse
        play("drive", square_pulse(duration="20us", amplitude="50mV" @ phase(deg=90)))
...
* **Square pulse** - constant amplitude, with optional linear rise/fall
...
* ``Voltage``, ``Magnitude`` - real voltages
* ``Angle`` / ``Phase`` - rotation angles/phase shifts
