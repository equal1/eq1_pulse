...
does not select the `A` (flake8-builtins) category, so `min`/`max` shadowing the builtins raises
nothing; the pre-commit `ruff` hook auto-removed the `# noqa: A002` markers as unused (`RUF100`) the
first time it ran, so the final code has no `noqa` at all. A private `_build_limits()` helper
assembles the `ValueLimits` from the flat `min`/`max`/`allowed` keywords and is shared by both
functions rather than duplicated. Both functions were also added to `builder/core.py`'s own
