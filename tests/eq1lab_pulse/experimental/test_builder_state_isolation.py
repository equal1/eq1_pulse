"""Builder state must not survive a schedule build, successful or otherwise.
...
def test_add_block_returns_a_usable_token():
    ...
    with experimental.build_schedule() as sched:
        token = experimental.add_block(block("q"), op_name="init")
        follow_up = experimental.play("q", square_pulse(**PULSE), ref_op=token, ref_pt="end")
