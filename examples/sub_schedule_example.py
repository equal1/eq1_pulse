...
def example_basic_sub_schedule():
    ...
    with build_schedule() as sched:
        ...
        with sub_schedule(op_name="init_block"):
...
def example_quantum_experiment_with_sub_schedules():
    ...
    with build_schedule() as main:
        ...
        # Gate sequence block
        with sub_schedule(op_name="gates", ref_op="initialization", ref_pt="end", rel_time="50ns"):
        ...
        # Measurement block
        with sub_schedule(op_name="measurement", ref_op="gates", ref_pt="end", rel_time="100ns"):
...
def example_parallel_operations_with_sub_schedules():
    ...
    with build_schedule() as parallel:
        ...
        with sub_schedule(op_name="qubit1_ops"):
        ...
        with sub_schedule(op_name="qubit2_ops", ref_pt_new="start"):
        ...
        with sub_schedule(op_name="joint_measurement", ref_op="qubit1_ops", ref_pt="end", rel_time="50ns"):
...
def example_reusable_sub_schedules():
    ...
    def create_init_block(op_name, qubit_channel):
        """Create a reusable initialization sub-schedule."""
        with sub_schedule(op_name=op_name):
    ...
    def create_measurement_block(op_name, drive_channel, readout_channel, var_name, **timing):
        """Create a reusable measurement sub-schedule."""
        with sub_schedule(op_name=op_name, **timing):
            play(drive_channel, square_pulse(duration="1us", amplitude="50mV"))
            record(readout_channel, var=var_name, duration="1us", integration=full_integration())

    # Use the reusable patterns
    with build_schedule() as multi_qubit:
        # Declare variables for measurement results
        var_decl("q0_result", "complex", unit="mV")
        var_decl("q1_result", "complex", unit="mV")
