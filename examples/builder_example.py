#!/usr/bin/env python3
"""Example demonstrating the builder interface for pulse sequences and schedules.

This script shows how to use the builder API to create pulse programs with:
- Simple sequences
- Schedules with relative positioning
- Control flow (loops, conditionals)
- Measurement operations

Note:
    String values like "10us" and "100mV" are used for readability in this example.
    In production code, you should use proper type constructors like Duration(us=10)
    or the models' automatic type conversion.
"""

# ruff: noqa: SIM117 RUF100

import sys
from pathlib import Path

from eq1_pulse.models import LinSpace

# Add src to path for development
sys.path.insert(0, str(Path(__file__).parent.parent / "src"))

from eq1_pulse.builder import *
from eq1_pulse.builder import experimental


def example_simple_sequence():
    """Example 1: Creating a simple pulse sequence."""
    print("=" * 70)
    print("Example 1: Simple Sequence")
    print("=" * 70)

    with build_sequence() as seq:
        # Play pulses on different channels
        play("drive", square_pulse(duration="10us", amplitude="100mV"))
        play("readout", sine_pulse(duration="5us", amplitude="50mV", frequency="5GHz"))

        # Barrier synchronizes channels - both must reach this point before continuing
        barrier("drive", "readout")

        # After barrier, operations on both channels start at the same time
        play("drive", square_pulse(duration="20us", amplitude="80mV"))
        play("readout", square_pulse(duration="20us", amplitude="40mV"))

    print(f"Created sequence with {len(seq.items)} operations")
    print(seq.model_dump_json(indent=2))
    print()
    return seq


def example_schedule_with_positioning():
    """Example 2: Creating a schedule with relative positioning."""
    print("=" * 70)
    print("Example 2: Schedule with Relative Positioning")
    print("=" * 70)

    with experimental.build_schedule() as sched:
        # First operation starts at default time
        op1 = experimental.play("qubit", square_pulse(duration="10us", amplitude="100mV"), op_name="drive_pulse")

        # Second operation starts 5us after the first one ends
        op2 = experimental.play(
            "qubit",
            square_pulse(duration="10us", amplitude="50mV"),
            ref_op=op1,
            ref_pt="end",
            rel_time="5us",
            op_name="second_pulse",
        )

        # Readout happens at the same time as second pulse
        experimental.wait("readout", duration="10us", ref_op=op2, ref_pt="start", ref_pt_new="start", rel_time=0)

    print(f"Created schedule with {len(sched.items)} operations")
    print(sched.model_dump_json(indent=2))
    print()
    return sched


def example_with_repetition():
    """Example 3: Using repetition (loop)."""
    print("=" * 70)
    print("Example 3: Repetition (Loop)")
    print("=" * 70)

    with build_sequence() as seq:
        # Repeat a pulse sequence 10 times
        with repeat(10):
            play("qubit", square_pulse(duration="50ns", amplitude="100mV"))
            wait("qubit", duration="50ns")

    print(f"Created sequence with {len(seq.items)} operation(s)")
    print(seq.model_dump_json(indent=2))
    print()
    return seq


def example_with_iteration():
    """Example 4: Using iteration (for loop)."""
    print("=" * 70)
    print("Example 4: Iteration (For Loop)")
    print("=" * 70)

    with build_sequence() as seq:
        # Iterate over frequency values
        var_decl("freq", "int", unit="MHz")
        with for_("freq", LinSpace(start=4000, stop=6000, num=100)):
            set_frequency("qubit", var("freq"))
            play("qubit", square_pulse(duration="100ns", amplitude="50mV"))
            wait("qubit", duration="100ns")

    print(f"Created sequence with {len(seq.items)} operation(s)")
    print(seq.model_dump_json(indent=2))
    print()
    return seq


def example_with_conditional():
    """Example 5: Using conditional."""
    print("=" * 70)
    print("Example 5: Conditional")
    print("=" * 70)

    with build_sequence() as seq:
        var_decl("result", "complex", unit="mV")
        # Measure first
        measure("readout", result_var="result", duration="1us", amplitude="50mV", integration=demod_integration())

        # Conditionally apply correction based on result
        with if_("result"):
            play("qubit", square_pulse(duration="50ns", amplitude="100mV"))

    print(f"Created sequence with {len(seq.items)} operation(s)")
    print(seq.model_dump_json(indent=2))
    print()
    return seq


def example_measurement():
    """Example 6: Measurement (simultaneous play + record)."""
    print("=" * 70)
    print("Example 6: Measurement Operation")
    print("=" * 70)

    with build_sequence() as seq:
        var_decl("result", "complex", unit="mV")
        var_decl("result_demod", "complex", unit="mV")
        # Simple measurement
        measure("readout", result_var="result", duration="1us", amplitude="50mV", integration=demod_integration())

        # Measurement with demodulation and phase
        # Using amplitude @ phase syntax for complex amplitude with phase
        measure(
            "readout",
            result_var="result_demod",
            duration="1us",
            amplitude="50mV" @ phase("90deg"),  #
            integration=demod_integration(),
        )

    print(f"Created sequence with {len(seq.items)} operation(s)")
    print(seq.model_dump_json(indent=2))
    print()
    return seq


def example_complex_program():
    """Example 7: Complex program combining multiple features."""
    print("=" * 70)
    print("Example 7: Complex Program")
    print("=" * 70)

    with build_sequence() as seq:
        # Initialize
        set_frequency("qubit", "5GHz")
        set_phase("qubit", "0deg")

        var_decl("amp", "int", unit="mV")
        var_decl("measurement", "complex", unit="mV")
        # Repeated Rabi experiment
        with repeat(100):
            # Sweep pulse amplitude
            with for_("amp", LinSpace(start=0, stop=200, num=10)):
                # Apply pulse with variable amplitude
                amp_ref = var("amp")
                play("qubit", square_pulse(duration="100ns", amplitude="1mV"), scale_amp=amp_ref)

                # Measure
                measure(
                    "readout",
                    result_var="measurement",
                    duration="1us",
                    amplitude="50mV",
                    integration=demod_integration(),
                )

                # Reset if needed
                with if_("measurement"):
                    play("qubit", square_pulse(duration="1us", amplitude="200mV"))

                # Wait between experiments
                wait("qubit", "readout", duration="10us")

    print(f"Created complex sequence with {len(seq.items)} operation(s)")
    print(seq.model_dump_json(indent=2))
    print()
    return seq


def main():
    """Run all examples."""
    print("\n")
    print("*" * 70)
    print("PULSE BUILDER INTERFACE EXAMPLES")
    print("*" * 70)
    print("\n")

    # Run all examples
    example_simple_sequence()
    example_schedule_with_positioning()
    example_with_repetition()
    example_with_iteration()
    example_with_conditional()
    example_measurement()
    example_complex_program()

    print("*" * 70)
    print("All examples completed successfully!")
    print("*" * 70)


if __name__ == "__main__":
    main()
