        ...
        with for_("amp", amplitude_sweep):
            ...
            play(
                ...
                square_pulse(
                    ...
                    amplitude=var("amp"),  # Variable amplitude in mV
                ),
            )

            # Measure qubit state via sensor (complex variable with demod)
            measure(
                "readout",
    ...
    with build_sequence() as seq:
        ...
        with for_("t_drive", duration_sweep):
            ...
            play(
                "qubit",
                square_pulse(
                    duration=var("t_drive"),  # Variable duration in ns
                    amplitude="80mV",  # Fixed amplitude
                ),
            )

            # Measure via sensor (complex variable with demod)
            measure(
                "readout",
                result_var="raw_result",
                duration="1us",
...
def example_frequency_rabi():
    ...
    with build_sequence() as seq:
        ...
        with for_("freq", freq_sweep):
            # Set drive frequency
            set_frequency("qubit", var("freq"))  # freq in GHz
        ...
        with for_("amp", amp_sweep):
            with for_("dur", dur_sweep):
                ...
                play(
                    "qubit",
                    square_pulse(
                        duration=var("dur"),  # Variable duration in ns
                        amplitude=var("amp"),  # Variable amplitude in mV
                    ),
                )
...
def example_rabi_with_detuning():
    ...
    with build_sequence() as seq:
        ...
        with for_("detuning", detuning_sweep):
            ...
            shift_frequency("qubit", var("detuning"))  # detuning in MHz
            ...
            with for_("t_pulse", duration_sweep):
                ...
                play(
                    "qubit",
                    square_pulse(
                        duration=var("t_pulse"),  # Variable duration in ns
                        amplitude="80mV",
                    ),
                )
