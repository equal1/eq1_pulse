...
class WaitForTrigger(ChannelOpBase):
    ...
    The channel must be a digital input line. Nothing in the IR enforces this; it is a property of
    the target's hardware configuration, same as every other channel property.

    This operation blocks only its own channel's timeline until the line goes high. It is **not** a
    barrier -- other channels continue independently. To make several channels wait on one trigger,
    combine ``barrier`` with a ``wait_for_trigger`` on each:

    .. code-block:: text

        barrier(a, b) ; wait_for_trigger(a) ; wait_for_trigger(b)
    """

    op_type: Literal["wait_for_trigger"] = "wait_for_trigger"
    """The type discriminator, always "wait_for_trigger"."""
