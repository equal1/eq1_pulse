...
**Wire form.** Every node is a JSON array, ``[<tag>, <operand>, ...]``, not an object. For the eight
operator nodes (:class:`UnaryExpr` through :class:`LenExpr`) ``<tag>`` is the operator itself --
``"+"``, ``"<"``, ``"abs"``, ``"[]"``, ``"len"`` -- and the rest of the array is its operand(s), in
declaration order. :class:`LiteralExpr`, :class:`SymbolExpr` and :class:`SweepExpr` hold no
operator, only a single payload field, so ``<tag>`` there is that field's own name -- ``"value"``,
``"symbol"``, or ``"sweep"`` -- and the array holds exactly one operand. :class:`ExprBase`
implements the array encoding once, generically over each node's declared fields, rather than per
class: adding a node type means declaring its fields, not hand-writing its wire form.
"""
...
class ExprBase(NoExtrasModel):
    ...
    (an operator, or -- for the three single-field leaf nodes -- the field's own name), and every
    ...
    def _operand_fields(cls) -> list[str]:
        ...
        Empty for :class:`LiteralExpr`, :class:`SymbolExpr` and :class:`SweepExpr`, whose one field
        *is* the tag field.
        """
...
class IndexExpr(ExprBase):
    ...
    field *name*, so the wire form is ``["[]", ["sweep", "vg"], [["symbol", {"var": "i"}]]]``.
